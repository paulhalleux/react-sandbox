...
function getEdgeCenter({
  ...
  // MODIFIED: ensure path break the nearest point to target
  const xOffset = direction === "h" ? offset : Math.abs(targetX - sourceX) / 2;
  const centerX = targetX < sourceX ? targetX + xOffset : targetX - xOffset;

  const yOffset = direction === "v" ? offset : Math.abs(targetY - sourceY) / 2;
